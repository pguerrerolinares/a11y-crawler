...
**Goal:** Optimize the audit probe pipeline with a generalized 4-tier system (Manifest → DOM/CSSOM → Interaction → LLM Vision) to reduce time by ~30-45%, cost by ~60-80%, and close 3 quality gaps vs Aiblu.
...
### Task 16: Integration test — run audit on kutxabankinvestment.es
...
curl -X POST http://localhost:3000/api/audits -H 'Content-Type: application/json' \
  -d '{"url":"https://kutxabankinvestment.es/","config":{"maxDepth":3,"maxPages":50,"wcagLevel":"AA"}}'
```
