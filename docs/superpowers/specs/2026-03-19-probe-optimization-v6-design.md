...
### Current Performance Breakdown (kutxabankinvestment.es, 33 pages)
...
**Problem**: On kutxabankinvestment.es, Tier 1 CSSOM resolved 0/351 elements because CSS is cross-origin. All promoted to Tier 2 — 1.1s wasted. Most modern sites using CDN-served CSS have this issue.
...
**File**: `src/worker/tier2.ts`
**Problem**: The v5 spec says Tier 2 evaluates popup persistence, hoverability, and dismissibility. But in the actual code, `detectPopup()` is called but the fields `popupPersistent`, `popupHoverable`, `popupDismissible` are never set. This explains the open gap from the Aiblu comparison (hover-focus in `/equipo/`).
...
4. **Implement hover-focus sub-tests in Tier 2** — closes Aiblu comparison gap
