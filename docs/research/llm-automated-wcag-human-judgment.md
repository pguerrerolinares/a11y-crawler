...
> Date: 2026-03-18
> Baseline: Comparison of Aiblu manual audit (kutxabankinvestment.es) vs a11y-crawler-v2 automated scan.
...
Manual accessibility audits (like Aiblu's report for Kutxabank Investment) cost thousands of euros, take weeks, and cover only a handful of pages. Our automated scanner (a11y-crawler-v2) covers 33+ pages with 2,596 issues detected — but misses ~25-30% of criteria that "require human judgment."

The question: **can we close that gap using LLMs, vision models, and advanced browser automation?**

### 1.2 Scope

We identified 8 WCAG criteria from the Aiblu report that our scanner doesn't cover, all traditionally considered to require human evaluation:
...
**Why it's considered "human judgment":** Aiblu flagged that menus and interactive controls don't expose `aria-expanded`, `aria-selected`, `aria-current` correctly. This requires interacting with the widget and verifying attribute changes.
...
Same approach as 2.4 (Kendall tau). The specific Aiblu finding was:
...
| Feature | axe-core | pa11y | WAVE | Aiblu (manual) | **a11y-crawler-v2 (proposed)** |
|---|---|---|---|---|---|
