...
```bash
ssh root@YOUR_VPS_IP "docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"
```

**Step 3: Verify memory usage**

```bash
ssh root@YOUR_VPS_IP "docker stats --no-stream --format 'table {{.Name}}\t{{.MemUsage}}\t{{.CPUPerc}}'"
```
...
**Step 5: Test with example-client.com (the working case)**
