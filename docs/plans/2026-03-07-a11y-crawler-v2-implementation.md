...
```bash
LLM_API_KEY=<key> bun run cli --url https://example-client.com --max-pages 10 --output test-report.json
```
...
```bash
git add -A && git commit -m "test: e2e validation with example-client.com"
```
