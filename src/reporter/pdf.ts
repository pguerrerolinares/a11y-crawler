...
export async function generatePdf(
  ...
  const html = buildHtml(report, wcagScore ?? null, cvdScreenshots);
...
${cvdScreenshots.length > 0 ? `
<h2>Color Vision Deficiency Analysis</h2>
<p style="font-size:12px;color:#555;margin-bottom:12px">
  WCAG 1.4.1 — Use of Color: Screenshots showing how the page appears under simulated color blindness.
  Significant visual differences may indicate reliance on color alone to convey information.
</p>
...
<div style="margin-bottom:24px">
  ...
  <div style="display:flex;gap:12px;align-items:flex-start">
    <div style="flex:1">
      ...
      <img src="data:image/png;base64,${ss.normalB64}" style="width:100%;border:1px solid #e5e7eb;border-radius:4px" />
    </div>
    <div style="flex:1">
      <p style="font-size:11px;color:#555;margin-bottom:4px">Simulated ${escHtml(ss.deficiency)}</p>
      <img src="data:image/png;base64,${ss.cvdB64}" style="width:100%;border:1px solid #e5e7eb;border-radius:4px" />
    </div>
  </div>
</div>`).join("")}` : ""}
